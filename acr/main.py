...
class ACR(nn.Module):
    def __init__(self, args_set=None):
        ...
        logging.info(
            "Loading {} renderer as visualizer, rendering size: {}".format(
                self.renderer, self.render_size
            )
        )
        self.visualizer = Visualizer(
            resolution=(self.render_size, self.render_size), renderer_type=self.renderer
        )
    ...
    def _build_model_(self):
        model = ACR_v1().eval()
        model = load_model(
            self.model_path, model, prefix="module.", drop_prefix="", fix_loaded=False
        )
    ...
    def process_results(self, outputs):
        ...
        if self.temporal_optimization:
            ...
            for idx, i in enumerate(outputs["detection_flag_cache"]):
                if i:
                    out_hand.append(
                        idx
                    )  # idx is also hand type, 0 for left, 1 for right
                else:
                    out_hand.append(-1)
            ...
            for sid, tid in enumerate(out_hand):
                if tid == -1:
                    continue
                (
                    outputs["params_dict"]["poses"][sid],
                    outputs["params_dict"]["betas"][sid],
                ) = smooth_results(
                    self.filter_dict[tid],
                    outputs["params_dict"]["poses"][sid],
                    outputs["params_dict"]["betas"][sid],
                )
        ...
        new_results = reorganize_results(
            outputs, outputs["meta_data"]["imgpath"], reorganize_idx
        )
    ...
    def forward(self, bgr_frame, path):
        ...
        if outputs is not None and outputs["detection_flag"]:
            ...
            results_dict, img_names, depth_map = self.visualizer.visulize_result_live(
                outputs,
                bgr_frame,
                outputs["meta_data"],
            ...
            img_name, mesh_rendering_orgimg = (
                img_names[0],
                results_dict["mesh_rendering_orgimgs"]["figs"][0],
            )
    ...
    def single_image_forward(self, bgr_frame, path):
        meta_data = img_preprocess(
            bgr_frame, path, input_size=args().input_size, single_img_input=True
        )
...
def main():
    ...
    if args().demo_mode == "image":
        ...
        # save outputs as pkl to itw_test_res
        with open("itw_test_res.pkl", "wb") as f:
    ...
    ###################### RUN on video ########################
        ...
        if not os.path.isdir(args().inputs):
            ...
            image_folder = (
                args().inputs[:-1] if args().inputs.endswith("/") else args().inputs
            )
        ...
        try:
            file_list = sorted(
                file_list, key=lambda x: int(os.path.basename(x).split(".")[0])
            )  # please ensure the image name is something like '000000.jpg'
        except:
