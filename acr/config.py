...
def parse_args(input_args=None):
    ...
    parser.add_argument(
        "--configs_yml",
        type=str,
        default=os.path.join(project_dir, "configs/demo.yml"),
        help="settings",
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default="pytorch3d",
        help="character: pytorch3d / pyrender",
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default="image",
        choices=["image", "video", "folder", "webcam"],
    )
    parser.add_argument(
        "--video_path", type=str, default=None, help="path to save the results"
    )
    parser.add_argument(
        "--video_folder", type=str, default=None, help="path to save the results"
    )
    ...
    mode_group.add_argument(
        "--model_version", type=int, default=1, help="model version"
    )
    mode_group.add_argument(
        "--multi_hand",
        type=bool,
        default=True,
        help="whether to make Multi-hand Recovery",
    )
    mode_group.add_argument(
        "--perspective_proj",
        type=bool,
    ...
    train_group.add_argument(
        ...
        type=float,
        default=0.1,
        help="factor for adjusting the lr",
    )
    train_group.add_argument(
        "--weight_decay", help="weight_decay", default=1e-6, type=float
    )
    train_group.add_argument("--epoch", type=int, default=120, help="training epochs")
    train_group.add_argument(
        "--fine_tune", type=bool, default=True, help="whether to run online"
    )
    ...
    train_group.add_argument(
        "--input_size", default=512, type=int, help="size of input image"
    )
    ...
    train_group.add_argument(
        "--optimizer_type", type=str, default="Adam", help="choice of optimizer"
    )
    train_group.add_argument(
        "--pretrain",
        type=str,
        default="simplebaseline",
        help="imagenet or spin or simplebaseline",
    )
    train_group.add_argument(
        "--fix_backbone_training_scratch",
        type=bool,
        default=False,
    ...
    train_group.add_argument(
        "--mix_training", type=str, default="single", choices=["single", "mix"]
    )
    ...
    train_group.add_argument(
        "--gt_scale", type=str2bool, default=False, help="whether to gt scale"
    )
    train_group.add_argument(
        "--intraction_field", type=str2bool, default=False, help="whether to gt scale"
    )
    ...
    ####################
    train_group.add_argument(
        "--part_seg_supervision",
        type=str,
        default="half",
        choices=["full", "no", "half"],
    )
    train_group.add_argument(
        "--part_seg_mode", type=str, default="digit", choices=["digit", "ours"]
    )
    train_group.add_argument(
        "--load_digit_seg", type=str, default="yes", choices=["yes", "no"]
    )
    train_group.add_argument(
        "--offset_mode",
        type=str,
        default="concat",
        choices=["offset", "replace", "concat"],
    )
    train_group.add_argument(
        "--attention_mode",
        type=str,
        default="pred-part",
        choices=["pred-part", "gt-part"],
    )

    # cross
    train_group.add_argument("--inter_prior", type=bool, default=True)
    train_group.add_argument(
        "--prior_mode", type=str, default="cross", choices=["cross"]
    )

    train_group.add_argument(
        "--eval_flip", type=str2bool, default=False, help="whether to run evaluation"
    )
    ...
    model_group.add_argument(
        ...
        type=str,
        default="hrnetv4",
        help="backbone model: resnet50 or hrnet",
    )
    model_group.add_argument(
        "--model_precision",
        type=str,
        default="fp16",
        help="the model precision: fp16/fp32",
    )
    model_group.add_argument("--deconv_num", type=int, default=0)
    model_group.add_argument(
        "--head_block_num", type=int, default=2, help="number of conv block in head"
    )
    ...
    model_group.add_argument(
        "--use_coordmaps", type=bool, default=True, help="use the coordmaps"
    )
    ...
    loss_group.add_argument(
        ...
        default=1000,
        type=float,
        help="max loss value for a single loss",
    )
    loss_group.add_argument(
        "--max_supervise_num",
        default=-1,
        type=int,
        help="max hand number supervised in each batch for stable GPU memory usage",
    )
    loss_group.add_argument("--supervise_cam_params", type=bool, default=False)
    loss_group.add_argument(
        "--match_preds_to_gts_for_supervision",
        type=bool,
        default=True,
        help="whether to match preds to gts for supervision",
    )
    loss_group.add_argument(
        "--matching_mode", type=str, default="all", help="all | random_one | "
    )
    loss_group.add_argument(
        "--supervise_global_rot",
        type=bool,
        default=False,
        help="whether supervise the global rotation of the estimated mano model",
    )
    loss_group.add_argument(
        "--HMloss_type",
        type=str,
    ...
    eval_group.add_argument(
        "--train_split", type=str, default="train", help="whether to run evaluation"
    )
    eval_group.add_argument(
        "--data_hand_type", type=str, default="all", help="whether to run evaluation"
    )
    eval_group.add_argument(
        "--eval_datasets",
        type=str,
        default="InterHand26M",
        help="whether to run evaluation",
    )
    eval_group.add_argument(
        "--val_batch_size", default=64, help="valiation batch_size", type=int
    )
    eval_group.add_argument(
        "--test_interval",
        default=5000,
        help="interval iteration between validation",
        type=int,
    )
    eval_group.add_argument(
        "--fast_eval_iter",
    ...
    eval_group.add_argument(
        ...
        type=bool,
        default=False,
        help="whether calculate PCK during evaluation",
    )
    eval_group.add_argument(
        "--PCK_thresh", type=int, default=150, help="training epochs"
    )
    ...
    maps_group.add_argument(
        "--centermap_size", type=int, default=64, help="the size of center map"
    )
    maps_group.add_argument(
        "--centermap_conf_thresh",
        type=float,
        default=0.35,
        help="the threshold of the centermap confidence for the valid subject",
    )
    maps_group.add_argument(
        "--collision_aware_centermap",
        type=bool,
        default=True,
    ...
    maps_group.add_argument(
        ...
        type=bool,
        default=True,
        help="center definition: keypoints or bbox",
    )

    distributed_train_group = parser.add_argument_group(
        title="options for distributed training"
    )
    distributed_train_group.add_argument(
    ...
    log_group.add_argument(
        "--model_path",
        type=str,
        default="./checkpoints/wild.pkl",
        help="trained model path",
    )
    log_group.add_argument(
        "--log-path",
        type=str,
        default=os.path.join(root_dir, "log/"),
    ...
    augmentation_group.add_argument(
        "--shuffle_crop_ratio_3d",
        default=0.9,
        type=float,
        help="the probability of changing the data loading mode from uncrop multi_hand to crop single hand",
    )
    augmentation_group.add_argument(
        "--shuffle_crop_ratio_2d",
        default=0.1,
        type=float,
        help="the probability of changing the data loading mode from uncrop multi_hand to crop single hand",
    )
    ...
    dataset_group.add_argument(
        "--dataset_rootdir",
        type=str,
        default=os.path.join(root_dir, "dataset/"),
        help="root dir of all datasets",
    )
    dataset_group.add_argument(
        "--dataset", type=str, default="internet", help="which datasets are used"
    )
    dataset_group.add_argument(
        "--voc_dir",
        type=str,
        default=os.path.join(root_dir, "dataset/VOCdevkit/VOC2012/"),
        help="VOC dataset path",
    )
    dataset_group.add_argument(
        "--max_hand", default=4, type=int, help="max hand number of each image"
    )
    dataset_group.add_argument(
        "--homogenize_pose_space",
        type=bool,
        default=False,
    ...
    dataset_group.add_argument(
        ...
        type=bool,
        default=True,
        help="wether use eft annotations for training",
    )
    ...
    mano_group.add_argument(
        ...
        type=str,
        default="6D",
        help="rotation representation type: angular, 6D",
    )
    mano_group.add_argument(
        "--rot_dim",
        type=int,
        default=6,
        help="rotation representation type: 3D angular, 6D",
    )
    mano_group.add_argument(
        "--cam_dim", type=int, default=3, help="the dimention of camera param"
    )
    ...
    mano_group.add_argument(
        ...
        type=int,
        default=10,
        help="the dimention of mano shape param, beta",
    )
    mano_group.add_argument(
        "--mano_theta_num",
        type=int,
        default=16,
        help="joint number of mano model we estimate",
    )
    mano_group.add_argument(
        "--mano_model_path", type=str, default="model_data/mano", help="mano model path"
    )

    mano_group.add_argument(
        "--mano_uvmap",
        type=str,
        default=os.path.join(model_dir, "parameters", "mano_vt_ft.npz"),
        help="mano UV Map coordinates for each vertice",
    )
    mano_group.add_argument(
        "--wardrobe",
        type=str,
        default=os.path.join(model_dir, "wardrobe"),
        help="path of mano UV textures",
    )

    mano_group.add_argument(
        "--lr_test", type=float, default=0.1, help="path to nvxia model"
    )
